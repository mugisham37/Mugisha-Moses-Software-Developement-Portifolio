 ...
 * 
 * A 3D card component with perspective effects and hover animations.
 * Creates an interactive "pin" effect with a floating card that rotates on hover.
 * 
 * @param {React.ReactNode} children - Content to display inside the pin container
 * @param {string} title - Title displayed in the floating label above the pin
 * @param {string} href - URL to navigate to when clicking the pin label
...
export const PinContainer = ({
  ...
  return (
    <div
      ...
      aria-labelledby={title ? `pin-title-${title.replace(/\s+/g, '-').toLowerCase()}` : undefined}
    >
      <div
        style={{
          perspective: "1000px",
 ...
 * 
 * Creates the visual perspective effect for the PinContainer.
 * Includes animated rings and connecting lines for visual appeal.
 * 
 * @param {string} title - Title displayed in the floating label
 * @param {string} href - URL to navigate to when clicking the label
 */
      ...
      <div className="w-full h-full -mt-7 flex-none inset-0">
        <div className="absolute top-0 inset-x-0 flex justify-center">
          <a
            ...
            className="relative flex space-x-2 items-center z-10 rounded-full bg-zinc-950 py-0.5 px-4 ring-1 ring-white/10 hover:ring-white/20 transition-all duration-300"
            id={title ? `pin-title-${title.replace(/\s+/g, '-').toLowerCase()}` : undefined}
